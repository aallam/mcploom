...
| [`codexec-process.ts`](./codexec-process.ts)                     | Run the same provider flow inside a child-process executor    |
| [`codexec-remote.ts`](./codexec-remote.ts)                       | Run the same provider flow through a transport-backed executor |
